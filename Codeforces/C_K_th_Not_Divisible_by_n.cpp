...
ll kthNonDivisibleNumber(ll N, ll K)
{
    return K + floor((K - 1) / (N - 1));
    // The key observation in the problem is that every number from 1 to N-1 is not divisible by N
    // and then Similarly, N + 1 to 2*N – 1 is also not divisible by N.
}
...
void solve()
{
    ...
    cout << kthNonDivisibleNumber(N, K) << endl;
}
